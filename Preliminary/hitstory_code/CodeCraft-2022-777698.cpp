...
//用于判断客户节点到边缘节点是否能够进行通讯，client_server_connect[i][j]表示第i个客户节点和第j个边缘节点是否能够通信
...
//    }
    ...
    getline(infile,line);
    client_server_connect = vector<vector<bool>>(clientNum,vector<bool>(serverNum));
    is.clear();
    is.str(line);
...
//                cout<<endl;
                ...
                for(int& neigh:server_list[serverId]){
                    curNeed+=curDemand[neigh];
                }
                if(serverLoad[serverId]/curNeed<10){    //10%以下就不用了，该参数也能够之后进行动态调整，例如计算比值之类
